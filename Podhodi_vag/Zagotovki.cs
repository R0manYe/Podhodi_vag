...
namespace Podhodi_vag
{
    class Zagotovki
    {
        public void Zag()
        {
            ...
            File.WriteAllText("ott100.xml", code1);
            ...
            "T.c.value('(Car/carNumber)[1]', 'varchar(40)') AS carNumber,T.c.value('(Station/StationId)[1]', 'int') AS StationId,T.c.value('(Station/StationCodeMod11)[1]', 'int') AS StationCode," +
            ...
            "T.c.value('(ToStation/StationName)[1]', 'varchar(100)') AS ToStationName,CONVERT (datetime, GETDATE())  FROM @x.nodes('/invoiceFiledCarsReply/invoiceFiledCar') T(c))";
